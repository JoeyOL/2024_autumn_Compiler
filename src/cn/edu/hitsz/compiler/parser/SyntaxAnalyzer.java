...
public class SyntaxAnalyzer {
    ...
    private Stack<Status> state_st = new Stack<>();
    private Stack<Symbol> sym_st = new Stack<>();
    ...
    public void loadTokens(Iterable<Token> tokens) {
        // TODO: 加载词法单元
        for (var item: tokens) this.tokens.addFirst(item);
...
//        throw new NotImplementedException();
    ...
    public void run() {
        // TODO: 实现驱动程序
        state_st.add(table.getInit());
        sym_st.add(new Symbol(Token.eof()));
        while (!tokens.empty()){
            var cur_tk = tokens.peek();
            var cur_st = state_st.peek();
            var act = cur_st.getAction(cur_tk);

            if (act.getKind() == Action.ActionKind.Shift) {
                callWhenInShift(cur_st, cur_tk);
                state_st.add(act.getStatus());
                sym_st.add(new Symbol(cur_tk));
                tokens.pop();
            }
            else if (act.getKind() == Action.ActionKind.Reduce) {
                callWhenInReduce(cur_st, act.getProduction());
                var prod = act.getProduction();
                for (int i = 0; i < prod.body().size(); i++) {
                    state_st.pop();
                    sym_st.pop();
                }
                state_st.add(table.getGoto(state_st.peek(), prod.head()));
                sym_st.add(new Symbol(prod.head()));
            }
            else if (act.getKind() == Action.ActionKind.Accept) {
                callWhenInAccept(cur_st);
                break;
            }
        }
